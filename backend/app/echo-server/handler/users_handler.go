...
func (h UsersHandler) LoginUser(e echo.Context) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "hashedPassword is not the hash of the given password") {
			log.Printf("Error on LoginUser request: %s", err.Error())
			return e.JSON(http.StatusBadRequest, fres.Response.StatusBadRequest("invalid password"))
		}

		if strings.Contains(err.Error(), "not found") {
			log.Printf("Error on LoginUser request: %s", err.Error())
			return e.JSON(http.StatusBadRequest, fres.Response.StatusBadRequest("user not found"))
		}
